...
var registerIndicatorCardNode = function () {
    ...
    g6_1.default.registerNode('indicator-card', {
        ...
        draw: function (cfg, group) {
            ...
            var _g = titleCfg !== null && titleCfg !== void 0 ? titleCfg : {}, titleStyle = _g.style, titleContainerStyle = _g.containerStyle;
            var _h = itemsCfg !== null && itemsCfg !== void 0 ? itemsCfg : {}, itemStyle = _h.style, itemContainerStyle = _h.containerStyle, layout = _h.layout, _j = _h.itemSpacing, itemSpacing = _j === void 0 ? 4 : _j, sort = _h.sort, _k = _h.padding, itemPadding = _k === void 0 ? [6, 0, 0] : _k;
            var itemPaddingArray = utils_1.getCssPadding(itemPadding);
            var _l = value, title = _l.title, items = _l.items;
            ...
            if (markerCfg) {
                ...
                var _m = shape.getBBox(), shapeWidth_1 = _m.width, shapeHeight_1 = _m.height;
                var _o = typeof markerCfg === 'function' ? markerCfg(cfg, group) : markerCfg, show = _o.show, _p = _o.position, position = _p === void 0 ? 'right' : _p, collapsed = _o.collapsed, markerStyle = _o.style;
        ...
        setState: function (name, value, item) {
            ...
            var stateName = util_1.isBoolean(value) ? name : name + ":" + value;
            ...
            if (value) {
                var _loop_1 = function (key) {
                    ...
                    if (util_1.isPlainObject(style) && !ARROWS.includes(key)) {
                ...
                // 所有生效的 state 的样式
                var enableStatesStyle = utils_1.cloneBesidesImg(item.getCurrentStatesStyle());
                var model = item.getModel();
                // 原始样式
                var originStyle_1 = util_1.mix({}, model.style, utils_1.cloneBesidesImg(item.getOriginStyle()));
                ...
                Object.keys(shapeAttrs_1).forEach(function (key) {
                    ...
                    if (attr && typeof attr === 'object') {
                        keyShapeStyles_1[key] = util_1.clone(attr);
                    }
                    else {
                ...
                var _loop_2 = function (p) {
                    var style = styles[p];
                    if (util_1.isPlainObject(style) && !ARROWS.includes(p)) {
                        var subShape_1 = group.find(function (element) { return element.get('name') === p; });
                        if (subShape_1) {
                            var subShapeStyles_1 = util_1.clone(subShape_1.attr());
                            util_1.each(style, function (v, key) {
                ...
                // 否则为普通对象
                if (!keyShapeName_1) {
                    util_1.mix(filtetDisableStatesStyle, keyShapeStyles_1);
                }
                else {
                ...
                for (var key in enableStatesStyle) {
                    ...
                    if (!util_1.isPlainObject(enableStyle) || ARROWS.includes(key)) {
                        // 把样式属性merge到keyShape中
                        if (!keyShapeName_1) {
                            util_1.mix(originStyle_1, (_a = {},
                                _a[key] = enableStyle,
                                _a));
                        }
                        else {
                            util_1.mix(originStyle_1[keyShapeName_1], (_b = {},
                                _b[key] = enableStyle,
                                _b));
                ...
                var originstyles = {};
                util_1.deepMix(originstyles, originStyle_1, filtetDisableStatesStyle, enableStatesStyle);
                var keyShapeSetted = false;
                var _loop_3 = function (originKey) {
                    var _d;
                    var style = originstyles[originKey];
                    if (util_1.isPlainObject(style) && !ARROWS.includes(originKey)) {
